...
class PhysicsGame {
  ...
  updateRespawns() {
    ...
    for (const [name, actor] of this.actors) {
      ...
      if (actor.state.respawn_time <= 0 && actor.state.invulnerable > 0) {
        actor.state.invulnerable = Math.max(0, actor.state.invulnerable - tickSeconds);
      }

      if (actor.state.respawn_time <= 0 && !this.paused) {
